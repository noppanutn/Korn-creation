...
<div class="bg">
  ...
  <div class="main_content">
    ...
    <table class="add_table" align="center" cellpadding = "10">
      <tr><th>Customer Name</th><th>Car Model</th><th>Price</th><th>Deal Date</th>
        <th>Payment Status</th><th>Shipping Status</th><th>Edit Order</th><th>Cancel Order</th></tr>
      <?php
      $salesmanid = $_SESSION['u_id'];
      $q = "SELECT C.CUSTOMER_FNAME,C.CUSTOMER_LNAME,M.MODEL, O.PRICE, O.dealDate
      , O.DEPOSIT_PAYMENT_STATUS, O.DELIVERY_STATUS, O.CAR_ORDER_ID
      FROM car_model as M, customer as C, car_order as O
      WHERE O.SALESMAN_ID = $salesmanid
      AND C.CUSTOMER_ID = O.CUSTOMER_ID
      AND M.CAR_ID = O.CAR_ID
      ORDER BY O.dealDate DESC";
      $result = $mysqli->query($q);
      while($row = $result->fetch_array()){
        //echo "something";
        echo "<tr><td>".$row['CUSTOMER_FNAME']." ".$row['CUSTOMER_LNAME']."</td>
        <td>".$row['MODEL']."</td><td>".number_format($row['PRICE'])."</td>
        <td>".$row['dealDate']."</td><td>".$row['DEPOSIT_PAYMENT_STATUS']."</td><td>".$row['DELIVERY_STATUS'].
        "</td><td><a href = 'edit_order.php?orderid=".$row['CAR_ORDER_ID']."'><img src='images/edit.png' height = '30px'></a></td>
        <td><a href = 'cancel_order.php?orderid=".$row['CAR_ORDER_ID']."'><img src='images/delete.png' height = '30px'></a></td></tr>";
      }
